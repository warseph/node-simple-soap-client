...
var parseResponse = function(data, callback) {
    ...
    var p = new expat.Parser('UTF-8'), body = false,
        response = {}, current = false, textOnly = ['val'];

    p.on('startElement', function(nsName, attrs) {
        var tag = parseTag(nsName), newElem, elem;
    ...
    p.on('text', function(string) {
        current.value = string;  // text nodes are store as .value properties (if the element has other attributes)
    });
    ...
    p.on('endElement', function(name) {
        ...
        if (body) {
            ...
            if (Object.keys(current) === textOnly) {
                current = current.val;
            } else {
